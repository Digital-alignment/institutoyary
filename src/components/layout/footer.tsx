        ...
        <footer className="bg-primary text-light-text">
            <div className="container mx-auto px-4 py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-3 text-sm text-light-text/80">
                        {settings.footer_layout?.address && (
                            <div className="flex items-start gap-2">
                                <MapPin className="h-4 w-4 mt-1 shrink-0" />
                                <span className="whitespace-pre-line">{settings.footer_layout.address}</span>
                            </div>
                        )}
                        {social.email && (
                            <div className="flex items-center gap-2">
                ...
                {/* Newsletter Column */}
                {settings.footer_layout?.show_newsletter && (
                    <div className="flex flex-col gap-4">
                        <h3 className="font-semibold text-lg">{settings.footer_layout.newsletter_title || "Fique por dentro"}</h3>
                        <p className="text-sm text-light-text/80">{settings.footer_layout.newsletter_text || "Receba novidades e ações em seu e-mail."}</p>
            ...
            <div className="border-t border-white/10">
                <div className="container mx-auto px-4 py-6 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-light-text/60">
                    <p>{settings.footer_layout?.copyright_text || `© ${new Date().getFullYear()} ${settings.site_title}. Todos os direitos reservados.`}</p>
